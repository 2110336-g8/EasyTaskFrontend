...
import { FileEdit, ImagePlus, Pencil, RefreshCcw, Save, Trash, X } from "lucide-react";
...
export default function ProfilePicture() {
    ...
    const schema: ZodType<UserImage> = z.object({
        file: z
            ...
            .refine((file) => file?.item(0)?.type.match('image'), 'Invalid file type')
    })
    ...
    useEffect(() => {
        const fetchUser = async () => {
            ...
            const res = await instance.get(`v1/users/${user._id}/profile-image`)
    ...
    const submitData = async (values: z.infer<typeof schema>) => {
        ...
        try {
            ...
            await instanceBinary.post(`v1/users/${user._id}/profile-image`, fileData, {
                ...
                {isEditing ?
                    <>
                        <Image
                            ...
                            src={preview === "" ? (currentimageURL === "" ? "/ProfilePicEmpty.png" : currentimageURL) : preview}
                            alt=""
                            width={160}
                            height={160}
                            priority>
                        ...
                        <Form {...form}>
                            <div className="flex flex-row gap-x-[8px] items-center">
                                {
                                    preview != '' &&
                                    <Button type='button' onClick={() => { form.reset(); setPreview(''); setDelete(false); }} className="h-[36px] px-[8px] gap-[8px] bg-slate-500">
                                        <RefreshCcw className="size-[24px]"></RefreshCcw>
                                        <p className="text-">Reset</p>
                                    </Button>
                                }
                                {
                                    preview == '' && currentimageURL != '' && !isDelete &&
                                    <Button type='button' onClick={() => { setDelete(true) }} className="h-[36px] px-[8px] gap-[8px] bg-slate-500">
                            ...
                            {form.formState.errors.file &&
                                <p className="text-p text-error-500">{form.formState.errors.file.message}</p>
                            }
                        </Form>
                    </>
                    :
                    <Image
                        className="size-[160px] rounded-[80px]"
                        src={currentimageURL === "" ? "/ProfilePicEmpty.png" : currentimageURL}
                        width={160}
                        height={160}
                        alt=""
                        priority>
